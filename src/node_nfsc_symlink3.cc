...
NFS::SymLink3Worker::SymLink3Worker(NFS::Client *client_,
    ...
    : Procedure3Worker(client_, (xdrproc_t)xdr_SYMLINK3res, callback),
      name(name_),
      path(path_)
{
...
void NFS::SymLink3Worker::procSuccess()
{
    v8::Local<v8::Value> obj_fh;
    if (res.SYMLINK3res_u.resok.obj.handle_follows) {
        obj_fh = Nan::NewBuffer(res.SYMLINK3res_u.resok.obj.post_op_fh3_u
                                .handle.data.data_val,
                                res.SYMLINK3res_u.resok.obj.post_op_fh3_u
    ...
    if (res.SYMLINK3res_u.resok.dir_wcc.before.attributes_follow)
        before = node_nfsc_wcc3(res.SYMLINK3res_u.resok.dir_wcc.before
                                .pre_op_attr_u.attributes);
    else
        before = Nan::Null();
    if (res.SYMLINK3res_u.resok.dir_wcc.after.attributes_follow)
        after = node_nfsc_fattr3(res.SYMLINK3res_u.resok.dir_wcc.after
    ...
    if (res.SYMLINK3res_u.resok.obj_attributes.attributes_follow)
        obj_attrs = node_nfsc_fattr3(res.SYMLINK3res_u.resok.obj_attributes
                                     .post_op_attr_u.attributes);
    else
        obj_attrs = Nan::Null();
...
void NFS::SymLink3Worker::procFailure()
{
    v8::Local<v8::Object> wcc = Nan::New<v8::Object>();
    v8::Local<v8::Value> before, after;
    if (res.SYMLINK3res_u.resfail.dir_wcc.before.attributes_follow)
        before = node_nfsc_wcc3(res.SYMLINK3res_u.resfail.dir_wcc.before
                                .pre_op_attr_u.attributes);
    else
        before = Nan::Null();
    if (res.SYMLINK3res_u.resfail.dir_wcc.after.attributes_follow)
        after = node_nfsc_fattr3(res.SYMLINK3res_u.resfail.dir_wcc.after
                                 .post_op_attr_u.attributes);
    else
        after = Nan::Null();
    wcc->Set(Nan::New("before").ToLocalChecked(), before);
